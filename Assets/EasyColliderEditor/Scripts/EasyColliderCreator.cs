  ...
  public class EasyColliderCreator
  {
    ...
    private List<EasyColliderData> CalculateRotateAndDuplicate(CREATE_COLLIDER_TYPE result, List<Vector3> worldVertices, EasyColliderRotateDuplicate ecrd)
    {
      ...
      for (int i = 0; i < ecrd.NumberOfDuplications; i++)
      {
        ...
        Quaternion rotation = RotationFromMatrix(baseData.Matrix);
        // Matrix4x4 m = Matrix4x4.TRS(attachTo.position, q * baseData.Matrix.rotation, baseData.Matrix.lossyScale);
        Matrix4x4 m = Matrix4x4.TRS(attachTo.position, q * rotation, mScale);

        //TODO: Rotated colliders with rotate and duplicate.
        switch (result)
        {
          case CREATE_COLLIDER_TYPE.CAPSULE:
          case CREATE_COLLIDER_TYPE.ROTATED_CAPSULE:
            {
              ...
              d2.Clone(d1);
              d2.Matrix = m;
              if (attachTo != ecrd.attachTo.transform)
              {
                Vector3 cShift = ecrd.attachTo.transform.position - attachTo.position;
                d2.Center += cShift;
              }
              data.Add(d2);
              break;
          ...
          case CREATE_COLLIDER_TYPE.CYLINDER:
            {
              ...
              if (attachTo != ecrd.attachTo.transform)
              {
                Vector3 cShift = (ecrd.attachTo.transform.position - attachTo.position) / ecrd.NumberOfDuplications;
                Vector3[] vertices = d2.ConvexMesh.vertices;
                for (int j = 0; j < vertices.Length; j++)
                {
                  vertices[j] += cShift;
                }
                d2.ConvexMesh.vertices = vertices;
              }
              d2.Matrix = m;
              data.Add(d2);
              break;
